...
public class MapServer {
    ...
    private static Location locates;
    ...
    public static void initialize() {
        String osmPath = MapServer.class.getResource(OSM_DB_PATH).getFile();
        graph = new GraphDB(osmPath);
        rasterer = new Rasterer();
        locates = new Location(graph.getNodesInBerkeley());
    }
    ...
    public static void main(String[] args) {
        ...
        post("/register", (req, res) -> {
            ...
            Map<String, Object> result = graph.getUserAuth().registerUser(username, password);
        ...
        post("/login", (req, res) -> {
            ...
            Map<String, Object> result = graph.getUserAuth().loginUser(username, password);
        ...
        /* Define get comments endpoint */
        get("/comments", (req, res) -> {
            String locationIdStr = req.queryParams("location_id");
            if (locationIdStr == null || locationIdStr.isEmpty()) {
                halt(HALT_RESPONSE, "Location ID is required");
            }
            ...
            try {
                ...
                halt(HALT_RESPONSE, "Invalid location ID");
                return "";
            }
            
            List<Map<String, Object>> comments = graph.getCommentsByLocation(locationId);
        ...
        /* Define add comment endpoint */
        post("/comments", (req, res) -> {
            String locationIdStr = req.queryParams("location_id");
            String userIdStr = req.queryParams("user_id");
            String content = req.queryParams("content");
            
            if (locationIdStr == null || userIdStr == null || content == null || 
                locationIdStr.isEmpty() || userIdStr.isEmpty() || content.isEmpty()) {
                halt(HALT_RESPONSE, "Missing required parameters");
            }
            ...
            try {
                ...
                halt(HALT_RESPONSE, "Invalid ID format");
                return "";
            }
            
            long commentId = graph.addComment(locationId, userId, content);
            
            Map<String, Object> result = new HashMap<>();
            result.put("success", commentId > 0);
    ...
    private static BufferedImage getImage(String imgPath) {
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            byte[] imageData = jedis.get(imgPath.getBytes());
            if (imageData != null) {
                ByteArrayInputStream bis = new ByteArrayInputStream(imageData);
//                System.out.println("cache hit");
                tileImg = ImageIO.read(bis);
                return tileImg;
            }
        } catch (Exception e) {
        ...
        if (tileImg == null) {
            try {
                java.net.URL resourceURL = MapServer.class.getResource(imgPath);
                if (resourceURL != null) {
                    tileImg = ImageIO.read(resourceURL);
                } else {
                    File in = new File("src/main/resources" + imgPath);
                    if (in.exists()) {
                ...
                try (Jedis jedis = jedisPool.getResource()) {
                    ...
                    ImageIO.write(tileImg, "png", bos);
                    byte[] imageData = bos.toByteArray();
                    jedis.set(imgPath.getBytes(), imageData);
                    // 设置过期时间为1小时
                    jedis.expire(imgPath.getBytes(), 3600);
                } catch (Exception e) {
                    System.out.println("Redis cache error: " + e.getMessage());
                }
    ...
    public static String toLowerOnly(String input) {
        if (input == null) {
            return "";
        }
        ...
        String lowercaseString = lettersOnly.toLowerCase();
    ...
    public static List<String> getLocationsByPrefix(String prefix) {
        prefix = toLowerOnly(prefix);
        List<String> res = locates.getLocationsByPrefix(prefix);
//        System.out.println(res);
    ...
    public static List<Map<String, Object>> getLocations(String locationName) {
        return locates.getLocations(locationName);
    }
